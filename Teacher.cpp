...
public:
	GoodSt(const std::string& n) : Student(n) {}
	Roots solve(const QuadEq& eq) const { return eq.solve(); }
};
...
public:
	BadSt(const std::string& n) : Student(n) {}
	Roots solve(const QuadEq& eq) const { return { 0.0 }; }
};
